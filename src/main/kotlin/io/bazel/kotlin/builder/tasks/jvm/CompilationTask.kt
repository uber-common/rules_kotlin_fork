  ...
  if (inputs.sourceJarsList.isEmpty()) {
    ...
    expandWithSources(
      SourceJarExtractor(
        ...
        fileMatcher = { str: String -> IS_JVM_SOURCE_FILE.test(str) || ("/$MANIFEST_DIR" in str && !("MANIFEST.MF" in str)) },
      ).also {
...
private fun JvmCompilationTask.expandWithSources(sources: Iterator<String>): JvmCompilationTask =
  updateBuilder { builder ->
    sources.copyServicesToGenClasses(directories).filterOutNonCompilableSources().partitionJvmSources(
